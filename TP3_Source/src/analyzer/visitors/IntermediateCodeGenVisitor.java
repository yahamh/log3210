...
public class IntermediateCodeGenVisitor implements ParserVisitor {
    ...
    public Object visit(ASTAssignStmt node, Object data) {
        String next = (String) data;
        data = null;
        Boolean isBool = SymbolTable.get(((ASTIdentifier) node.jjtGetChild(0)).getValue()) == VarType.Bool;
        String _L1 = "";
        String _L2 = "";
        if(isBool) {
            _L1 = genLabel();
    ...
    public Object codeExtAddMul(SimpleNode node, Object data, Vector<String> ops) {
        ...
        for (int i = 1; i < node.jjtGetNumChildren(); i++) {
            String _old_t = _t;
            _t = genId();
            String oldValue = node.jjtGetChild(i-1).jjtAccept(this, data).toString();
            value = node.jjtGetChild(i).jjtAccept(this, data).toString();
            if(i==1) m_writer.print(_t+" = "+oldValue+" "+ops.get(i-1)+" "+value+"\n");
            if(i>1) m_writer.print(_t+" = "+_old_t+" "+ops.get(i-1)+" "+value+"\n");
        }
        return _t;
    }
    ...
    public Object visit(ASTIdentifier node, Object data) {
        ...
        if(SymbolTable.get(value) == VarType.Bool) {
